  ...
  return (
    <div className="relative w-full aspect-[4/5] bg-emerald-950 border border-emerald-800/60 rounded-2xl overflow-hidden shadow-2xl shadow-black/40 select-none">
      ...
      {draftSlots.map((slot) => {
        ...
        let ratingColor = "bg-zinc-500 text-white";
        if (playerRating >= 85) ratingColor = "bg-amber-500 text-black font-extrabold shadow-amber-500/50 shadow-md";
        else if (playerRating >= 75) ratingColor = "bg-yellow-400 text-black font-semibold";
        else if (playerRating >= 65) ratingColor = "bg-zinc-300 text-black";
        ...
        return (
          ...
          >
            {hasPlayer ? (
              ...
              <div 
                className={`relative flex flex-col items-center p-1.5 w-16 sm:w-20 rounded-xl border transition-all duration-300 ${
                  isActive 
                    ? "bg-saweria/10 border-saweria shadow-[0_0_15px_rgba(255,203,5,0.4)] scale-105" 
                    : "bg-zinc-950/80 border-zinc-800 hover:border-saweria hover:bg-zinc-950 hover:shadow-[0_0_12px_rgba(255,203,5,0.2)] hover:scale-105"
                }`}
              >
                {/* Rating Badge */}
                ...
                {/* Player Face/Avatar */}
                <div className="relative w-8 h-8 sm:w-11 sm:h-11 rounded-full overflow-hidden bg-zinc-900 border border-zinc-800 flex items-center justify-center mt-1">
                  {slot.player?.img_url && slot.player.img_url.includes("default") === false ? (
                    ...
                    <User className="w-5 h-5 sm:w-7 sm:h-7 text-zinc-600" />
                  )}
                </div>

                {/* Player Name */}
                <span className="mt-1 text-[9px] sm:text-[11px] font-medium text-white truncate max-w-full text-center px-0.5">
                  {slot.player?.name.split(" ").slice(-1)[0]}
                </span>
                
                {/* Player Role/Position */}
                <span className="text-[8px] font-bold uppercase text-saweria-light scale-90 sm:scale-100">
                  {slot.position}
                </span>
              </div>
            ) : (
              // Empty Position Slot
              <div 
                className={`flex flex-col items-center justify-center w-12 h-12 sm:w-16 sm:h-16 rounded-full border-2 border-dashed transition-all duration-300 ${
                  isActive 
                    ? "border-saweria bg-saweria/20 shadow-[0_0_15px_rgba(255,203,5,0.4)] scale-105" 
                    : "border-white/30 bg-black/40 hover:border-saweria hover:bg-saweria/10 hover:scale-105"
                }`}
              >
                <Plus className={`w-4 h-4 sm:w-5 sm:h-5 ${isActive ? "text-saweria animate-pulse" : "text-white/40 group-hover:text-saweria"}`} />
                <span className={`text-[8px] sm:text-[10px] font-bold ${isActive ? "text-saweria" : "text-white/60 group-hover:text-saweria"}`}>
                  {slot.position}
                </span>
              </div>
            )}
